import * as schemas from '@app/Backend/Schemas/index.ts'
import CompanyModule from '@app/Company/index.ts'
import { db } from '@app/Database.ts'

/**
 * Sync company dividends.
 * @description Updates dividend announcement schedules.
 * @param year - Target year
 * @param month - Target month
 * @returns Empty promise completion
 */
export async function syncCompanyDividend(year: number, month: number): Promise<void> {
  const module = new CompanyModule()
  const result = await module.getDividendAnnouncements(year, month)
  if (result && result.data.length > 0) {
    const queries = result.data.map((item) => {
      const id = `${item.code}-${new Date(item.recordDate).getTime()}`
      const values = {
        id,
        code: item.code,
        name: item.name,
        cashDividend: item.cashDividend,
        cumDividend: item.cumDividend ? new Date(item.cumDividend) : null,
        exDividend: item.exDividend ? new Date(item.exDividend) : null,
        recordDate: new Date(item.recordDate),
        paymentDate: item.paymentDate ? new Date(item.paymentDate) : null
      }
      return db.insert(schemas.companyDividend).values(values).onConflictDoUpdate({
        target: schemas.companyDividend.id,
        set: values
      })
    })
    await Promise.all(queries)
  }
}
